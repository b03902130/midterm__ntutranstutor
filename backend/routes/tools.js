...
let tools = {
    ...
    docs = docs.map(doc => {
      return {
        ...
        department: !doc.departmentid.name
          ? preload.departmentInfo.id2name[doc.departmentid.toString()]
          : {
  ...
  organizeOutputCourse: function(docsFromDatabase) {
    ...
    docs = docs.map(doc => {
      return {
        id: doc.id,
        teacher: doc.teacherid.name
          ? tools.organizeOutputTeacher([doc.teacherid])
          : doc.teacherid.toString(),
        subject: !doc.subjectid.name
          ? preload.subjectInfo.id2name[doc.subjectid.toString()]
          : {
              id: doc.subjectid.id,
